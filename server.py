        ...
        # Mise à jour de la fiche d'identité
            ...
            c.execute("""
                ...
                ON CONFLICT(profile_id) DO UPDATE SET
                    ...
                    taille = COALESCE(excluded.taille, identity_cards.taille),
                    poids = COALESCE(excluded.poids, identity_cards.poids),
                    pointure = COALESCE(excluded.pointure, identity_cards.pointure),
                    tour_poitrine = COALESCE(excluded.tour_poitrine, identity_cards.tour_poitrine),
                    tour_taille = COALESCE(excluded.tour_taille, identity_cards.tour_taille),
                    tour_hanches = COALESCE(excluded.tour_hanches, identity_cards.tour_hanches),
                    mensurations = CASE WHEN excluded.mensurations != '' THEN excluded.mensurations ELSE identity_cards.mensurations END,
                    origines = CASE WHEN excluded.origines != '' THEN excluded.origines ELSE identity_cards.origines END,
                    couleur_cheveux = CASE WHEN excluded.couleur_cheveux != '' THEN excluded.couleur_cheveux ELSE identity_cards.couleur_cheveux END,
                    style = CASE WHEN excluded.style != '' THEN excluded.style ELSE identity_cards.style END,
